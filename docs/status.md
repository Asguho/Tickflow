...
This is not yet release-Done. The remaining work is primarily device/emulator verification, deeper persistence/service tests, stronger correction UI, and lifecycle hardening for Android foreground-service restrictions.
...
| Item | Status | Notes |
| --- | --- | --- |
| MVP runtime hardening | In progress | Service and Wi-Fi coordinator compile and build, but require real device/emulator validation. |
...
| Area | Notes |
| --- | --- |
| Emulator/device launch verification | Not run in this environment. |
...
| Foreground service lifecycle tests | Presence event processing has fake-based unit coverage; foreground service lifecycle still requires instrumentation/manual verification. |
| Wi-Fi automatic tracking manual test | Requires a device/emulator scenario with permissions and configured Wi-Fi. |
...
| --- | --- |
| Configure office Wi-Fi and work schedule | Implemented in setup/settings with weekday default schedule. |
| Automatic Wi-Fi tracking | Implemented through connectivity callback and coordinator; needs device validation. |
| Manual start/stop/edit | Start/stop and date-selectable session add/edit/delete are implemented. |
| Persist sessions across restart | Room persistence implemented; recovery path needs device validation. |
| Foreground tracking notification | Implemented; notification actions include stop/open app. |
| Carry-over calculation | Domain logic implemented and unit-tested; Home derives carry-in from the last 30 days of persisted sessions. |
...
| Order | Task | Output |
| --- | --- | --- |
| 1 | Run emulator/device checklist | Use `docs/manual-qa.md` for fresh install, permissions, manual tracking, notification action, Wi-Fi tracking, process death, and reboot path. |
| 2 | Add instrumentation coverage where feasible | Cover actual notification actions, active recovery, and foreground service lifecycle on Android runtime. |
